...
function model(dY, Y, p, t)
    ...
    dY[3] = epsilon*β*ϕ*S*Vi - δ*Vi
    dY[4] = (1-epsilon)*β*ϕ*S*Vi - δ*Vd
end
...
plot!(pl_sim[1], sol_test.t, sol_test[1,:]+sol_test[2,:], label="H", xlabel="time (days)",
      ylabel="abundances (cell/ml)", legend=:bottomright, yscale=:log10, lw=3)
plot!(pl_sim[2], sol_test.t, sol_test[3,:]+sol_test[4,:], label="V", xlabel="time (days)",
...
## ===== MULTI-RUN =====
lower        = log.([1E-15, 1e-10, 1e-10])
upper        = log.([1E-6, 1e-3, 1e-6])
...
for cycle in 1:n_cycles
    ...
    u0_mean = [H0_mean * prev_S_frac, H0_mean * prev_R_frac, V0_mean * prev_Vi_frac, V0_mean * prev_Vd_frac]
    ...
    Vi_tf = sol_c[3, end]
    Vd_tf = sol_c[4, end]
    total_V_tf = Vi_tf + Vd_tf
    global prev_Vi_frac = total_V_tf > 0 ? Vi_tf / total_V_tf : 1.0
    global prev_Vd_frac = total_V_tf > 0 ? Vd_tf / total_V_tf : 0.0

    lbl = cycle == 1 ? "Model" : ""
    plot!(pl_fit[1], sol_c.t, sol_c[1,:] .+ sol_c[2,:], label=lbl, color=model_color, lw=4, alpha=0.7)
    plot!(pl_fit[2], sol_c.t,  sol_c[3,:] .+ sol_c[4,:], label=lbl, color=model_color, lw=4, alpha=0.7)
end
...
display(pl_fit)
savefig(pl_fit, joinpath(@__DIR__, "060526_output/SRViVd_model.png"))
...
function objective(θ)
    ...
    for (key, cyc) in cycles
        ...
        if cycle ==1
            ...
            tH = cycle == 1 ? cyc.tH[1:6] : cyc.tH
            H  = cycle == 1 ? cyc.H[1:6]  : cyc.H
            tV = cycle == 1 ? cyc.tV[1:6] : cyc.tV
            V  = cycle == 1 ? cyc.V[1:6]  : cyc.V
            ...
            V_pred = [max(sol_cyc(t)[2], 1e-12) for t in tV]
...
# Un segment de modèle par cycle, repartant de la moyenne des CI de ce cycle
for cycle in 1:n_cycles
...
display(pl_fit)
savefig(pl_fit, joinpath(@__DIR__, "020526_output/SV_model.png"))
...
t_H = df_H[6:end,1]./24; H = df_H[6:end,2]
t_V = df_V[6:end,1]./24; V = df_V[6:end,2]
...
function objective_DE(θ)
    ...
    prob_new = ODEProblem(model_SV_no_delta_only_phi, Y0, tspan, p)
...
pl_data = plot(layout=(1,2), size=(700,250), margins=5mm)
scatter!(pl_data[1],t_H,H,label="host data",xlabel="time (days)", ylabel="abundances (cell/ml)", yscale=:log10,legend=:bottomright)
scatter!(pl_data[2],t_V,V,label="virus data",xlabel="time (days)", ylabel="abundances (virus/ml)", yscale=:log10,legend=:topleft)
plot!(pl_data[1],sol.t,sol[1,:],label="host model",lw=3)
plot!(pl_data[2],sol.t,sol[end,:],label="virus model",xlabel="time (days)", ylabel="abundances (virus/ml)",yscale=:log10,lw=3)

println("Saving plot...")
savefig(pl_data, joinpath(@__DIR__, "170426_output/cycle1_2_model_SV_only_phi_no_delta_fit_phi.png"))
## 1st, you have to be careful on the names you are using. Do not use () or ° in the names of your files. Only plain text with underscores are allowed. Otherwise it can creates some issues. 
...
t_H = df_H[1:6,1]; H = df_H[1:6,2]
t_V = df_V[1:6,1]; V = df_V[1:6,2]
...
plot!(pl_sim[1],sol.t,sol[1,:]+sol[2,:]+sol[3,:]+sol[4,:]+sol[5,:]+sol[6,:],label="tot. H.",xlabel="time (days)", ylabel="abundances (cell/ml)",yscale=:log10,lw=3,legend=:bottomleft)
plot!(pl_sim[2],sol.t,sol[7,:],label="V",xlabel="time (days)", ylabel="abundances (virus/ml)",legend=:bottomright,yscale=:log10,lw=3)


## ===== FITS =====

Y0 = [H[1],0,0,0,0,0,V[1]]
tspan = (t_H[1],t_H[end])
...
function objective_DE(θ)
    ...
    S_pred = [max(y[1] + y[2] + y[3] + y[4] + y[5] + y[6], 1e-12) for y in YH]
    V_pred = [max(y[7], 1e-12) for y in YV]
...
lower = log.([0.1, 1E7, 1E-12, 1E-15, 10.0, 1E-3, 1e-8])
upper = log.([0.7, 1E8, 1E-1,  1E-6, 500.0, 10,   0.1])
...
scatter!(pl_data[1],t_H,H,label="host data",xlabel="time (days)", ylabel="abundances (cell/ml)", yscale=:log10)
scatter!(pl_data[2],t_V,V,label="virus data",xlabel="time (days)", ylabel="abundances (virus/ml)", yscale=:log10,legend=:bottomright)
plot!(pl_data[1],sol.t,sol[1,:]+sol[2,:]+sol[3,:]+sol[4,:]+sol[5,:]+sol[6,:],label="Hmod",lw=3)

...
# ===== PHI CHANGE LINES =====
phi_changes = [t_H2[1], t_H6[1], t_H8[1], t_H10[1]]
...
phi_vals  = [ϕ1, ϕ2, ϕ3, ϕ4, ϕ5, ϕ6]
beta_vals = [β1, β2, β3, β4, β5, β6]

phi_cycle_labels = ["c1", "c1–2", "c2–3", "c3–4", "c4–5", "c5"]

# ===== Régression linéaire en log-log =====
log_beta = log10.(beta_vals)
log_phi  = log10.(phi_vals)

n    = length(log_beta)
x̄    = mean(log_beta)
ȳ    = mean(log_phi)
slope    = sum((log_beta .- x̄) .* (log_phi .- ȳ)) / sum((log_beta .- x̄).^2)
intercept = ȳ - slope * x̄

# R²
ŷ    = slope .* log_beta .+ intercept
ss_res = sum((log_phi .- ŷ).^2)
ss_tot = sum((log_phi .- ȳ).^2)
R2   = 1 - ss_res / ss_tot

# Courbe de régression sur la plage x
x_reg_log = range(log10(minimum(beta_vals) / 2), log10(maximum(beta_vals) * 4), length=200)
y_reg_log = slope .* x_reg_log .+ intercept
x_reg = 10 .^ x_reg_log
y_reg = 10 .^ y_reg_log
...
p_phi_beta = scatter(
    ...
    color=:black,
    xlabel="β (-)",
    ylabel="ϕ (mL/cell/day)",
    xscale=:log10,
    yscale=:log10,
    legend=false,
    grid=true,
    size=(900,600),
    xticks=(ytick_vals4, ytick_labels4),
    yticks=(ytick_vals3, ytick_labels3),
    margins=10mm,
    ytickfontsize=26,
    legendfontsize=20,
...
# ===== Annotations labels cycles =====
i_rightmost = argmax(beta_vals)
placed = Float64[]
...
for i in 1:length(beta_vals)
    ...
    min_gap = 2
    for y_placed in sort(placed)
        if abs(log10(y_annot) - log10(y_placed)) < log10(min_gap)
            y_annot = y_placed * min_gap
        end
    end
    ...
    annotate!(p_phi_beta,
        x_annot,
        y_annot,
        text(phi_cycle_labels[i], 24, :black, halign)
    )
end

# ===== Annotation équation + R² =====
slope_str     = round(slope, digits=2)
intercept_str = intercept >= 0 ? "+ $(round(intercept, digits=2))" : "- $(round(abs(intercept), digits=2))"
R2_str        = round(R2, digits=2)

eq_line  = "log₁₀(ϕ) = $(slope_str) · log₁₀(β) $(intercept_str)"
r2_line  = "R² = $(R2_str)"

xlims!(p_phi_beta, minimum(beta_vals) / 2,  maximum(beta_vals) * 4)
ylims!(p_phi_beta, minimum(phi_vals)  / 2,  maximum(phi_vals)  * 3)

# Position en bas à droite (coordonnées de données, espace log)
x_eq = maximum(beta_vals) * 2.5
y_eq = maximum(phi_vals) * 2.5

annotate!(p_phi_beta, x_eq, y_eq,
    text(eq_line * "\n" * r2_line, 18, :gray40, :right)
)

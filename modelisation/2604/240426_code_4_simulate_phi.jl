...
poly_file = joinpath(@__DIR__, "240426_output/$(n_interv)_polynome.txt")
φ_global = parse_polynome(poly_file)
...
# plot S, V
p1 = plot(yscale=:log10, xlabel="t", ylabel="S(t)", legend=false)
p2 = plot(yscale=:log10, xlabel="t", ylabel="V(t)", legend=false)

scatter!(p1, t_H, H)
scatter!(p2, t_V, V)
for sol in sols
    plot!(p1, sol.t, sol[1,:], lw=2)
    plot!(p2, sol.t, sol[2,:], lw=2)
end
plot(p1, p2, layout=(1,2), size=(1200,450), margins=10mm)




## ===== PLOTS =====
p3 = plot(yscale=:log10, xscale=:log10, xlabel="S(t)", ylabel="phi(t)")
p4 = plot(yscale=:log10, xscale=:log10, xlabel="V(t)", ylabel="phi(t)")

ytick_vals   = [10.0^i for i in -14:1:-7]
ytick_labels = [L"10^{%$i}" for i in -14:1:-7]
...
for (i, sol) in enumerate(sols)
    plot!(p3, sol[1,:], [φ_global(t) for t in sol.t], lw=2, yscale=:log10, label="Cycle $i", xlims=(1e3, 1e8), xticks=(xtick_vals1, xtick_labels1))
    plot!(p4, sol[2,:], [φ_global(t) for t in sol.t], lw=2, yscale=:log10, label="Cycle $i", xlims=(1e6, 1e10), xticks=(xtick_vals2, xtick_labels2))
    scatter!(p3, [sol[1,1]], [φ_global(sol.t[1])], color=:black, label="")
    scatter!(p4, [sol[2,1]], [φ_global(sol.t[1])], color=:black, label="")
end

plot(p3, p4, plot_title="$(n_interv+1) intervalles par cycle", layout=(1,2), size=(1200,450), margins=10mm, legend=:bottomleft, ylims=(1e-14, 1e-7), yticks=(ytick_vals, ytick_labels))
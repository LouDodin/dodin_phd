...
## Seff and Veff
V4 = [ 1.7503448275862068e7,
5.2947931034482755e7,
 1.9885862068965517e7,
 3.345103448275862e7,
 1.0384596814752725e8,
 1.9414033528918692e8]

H8 = [ 1.2977924840933193e7,
 2.6574975e7,
 5.6186007720438845e7,
 6.80011032388664e7,
 5.540576086956521e7]

H10 = [
 3.7689967455621306e6,
 1.2632464033481559e7,
 1.681891544117647e7,
 2.7821590821935322e7,
 2.5312939024390247e7,
 3.7771289697908595e7]

using Plots

# Effective values
H1eff = sum(H1 .* V1) / sum(V1)
V1eff = sum(H1 .* V1) / sum(H1)

H23eff = (sum(H2 .* V2) + sum(H3 .* V3)) / (sum(V2) + sum(V3))
V23eff = (sum(H2 .* V2) + sum(H3 .* V3)) / (sum(H2) + sum(H3))

H45eff = (sum(H4 .* V4) + sum(H5 .* V5)) / (sum(V4) + sum(V5))
V45eff = (sum(H4 .* V4) + sum(H5 .* V5)) / (sum(H4) + sum(H5))

H67eff = (sum(H6 .* V6) + sum(H7 .* V7)) / (sum(V6) + sum(V7))
V67eff = (sum(H6 .* V6) + sum(H7 .* V7)) / (sum(H6) + sum(H7))

H89eff = (sum(H8 .* V8) + sum(H9 .* V9)) / (sum(V8) + sum(V9))
V89eff = (sum(H8 .* V8) + sum(H9 .* V9)) / (sum(H8) + sum(H9))

H10eff = sum(H10 .* V10) / sum(V10)
V10eff = sum(H10 .* V10) / sum(H10)

# Lists
phi_list = [ϕ1, ϕ2, ϕ3, ϕ4, ϕ5, ϕ6]
Heff_list = [H1eff, H23eff, H45eff, H67eff, H89eff, H10eff]
Veff_list = [V1eff, V23eff, V45eff, V67eff, V89eff, V10eff]

cycle = [1, 2, 3, 4, 5, 6]

# Red → Yellow gradient
cmap = cgrad([:yellow, :red])

# Bigger marker size
ms = 10

# Scatter plots
scatter(Heff_list, phi_list,
    xlabel = "Seff",
    ylabel = "phi",
    yscale = :log10,
    marker_z = cycle,
...
scatter(cycle, phi_list ./ Heff_list,
    xlabel = "cycle",
    ylabel = "ratio phi/H",
    yscale = :log10,
    marker_z = cycle,
...
scatter(cycle, phi_list ./ Veff_list,
    xlabel = "cycle",
    ylabel = "ratio phi/V",
    yscale = :log10,
    marker_z = cycle,
...
# ===== PHI SEGMENTS =====
phi_segments = [
    (t_start=t_H1[1],  t_end=t_H1[end],  phi=ϕ1),

    (t_start=t_H2[1],  t_end=t_H2[end],  phi=ϕ2),
    (t_start=t_H3[1],  t_end=t_H3[end],  phi=ϕ2),

    (t_start=t_H4[1],  t_end=t_H4[end],  phi=ϕ3),
    (t_start=t_H5[1],  t_end=t_H5[end],  phi=ϕ3),

    (t_start=t_H6[1],  t_end=t_H6[end],  phi=ϕ4),
    (t_start=t_H7[1],  t_end=t_H7[end],  phi=ϕ4),

    (t_start=t_H8[1],  t_end=t_H8[end],  phi=ϕ5),
    (t_start=t_H9[1],  t_end=t_H9[end],  phi=ϕ5),

    (t_start=t_H10[1], t_end=t_H10[end], phi=ϕ6),
]

...
const FITTED_PARAMS = [
    (name = :eps_V,   lower = 1e-10, upper = 1, description = "total rate (α+β)"),
    (name = :K_V,   lower = 1e-10, upper = 1e10, description = "K_V"),
    (name = :ϕ,   lower = 1e-14, upper = 1e-7, description = "interaction rate"),
]

# ── ODE ─────────────────────────────────────────────────────────────────────
function ODE_MODEL!(dY, Y, p_vec, t)
    eps_V, K_V, ϕ = p_vec[1], p_vec[2], p_vec[3]
    S, R, Vi = Y[1], Y[2], Y[3]
    H = S + R
    r, K, β, δ = FIXED_PARAMS.r, FIXED_PARAMS.K, FIXED_PARAMS.β, FIXED_PARAMS.δ
    alpha = eps_V*Vi/(K_V + Vi)
    gamma = eps_V*K_V/(K_V + Vi)

...
## ===== Select model =====
const MODEL_FILE = joinpath(@__DIR__, "models/model_SViVd.jl")
...
ytick_vals2   = [10.0^i for i in 3:10]
ytick_labels2 = [L"10^{%$i}" for i in 3:10]
...
for rep in replicates
    for cyc_idx in 1:cycles_sim
        ...
        scatter!(pl_fit[2], data.tV, data.V;
            label=lbl, color=replicate_colors[rep], alpha=0.7,
            ylabel="Virus\n[part/mL]",
            ylims=(1e3, 1e10), yticks=(ytick_vals2, ytick_labels2),
            markershape=:circle, markersize=8,
            legend=(0.15, 0.25))
    end
end
...
for cycle in 1:cycles_sim
    ...
    for sl in ModelDef.STATE_LABELS
        if sl.state == :Vi
            lbl_Vi = cycle == 1 ? "Model Vi" : ""
            plot!(pl_fit[2], sol.t, sl_vals(sl), label=lbl_Vi, color=sl.color, lw=sl.lw, ls=sl.ls, alpha=0.7)
        elseif sl.state == :Vd
            lbl_Vd = cycle == 1 ? "Model Vd" : ""
            plot!(pl_fit[2], sol.t, sl_vals(sl), label=lbl_Vd, color=sl.color, lw=sl.lw, ls=sl.ls, alpha=0.7)
        end
    end
end

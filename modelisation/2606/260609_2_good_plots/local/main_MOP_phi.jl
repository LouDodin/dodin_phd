...
for cycle in 1:cycles_sim
    ...
    MOP_vals = sol[2,:] ./ sol[1,:]

    # Filter out null values (zeros, infinities, NaNs)
    valid_idx = findall(x -> x > 0 && isfinite(x), phi_vals) ∩ findall(x -> x > 0 && isfinite(x), MOP_vals)
    phi_vals = phi_vals[valid_idx]
    MOP_vals = MOP_vals[valid_idx]

    plot!(pl_fit, MOP_vals, phi_vals, label="Cycle $cycle", color=colors[cycle], lw=3, alpha=0.7,
    xlims=(1e-1, 1e6), xticks=(ytick_vals_mop, ytick_labels_mop),
...
fig_path = joinpath(@__DIR__, "output/MOP_vs_phi_$(ϕ_combi).png")

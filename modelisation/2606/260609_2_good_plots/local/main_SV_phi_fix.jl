...
## ===== Select model =====
const MODEL_FILE = joinpath(@__DIR__, "models/model_SVi.jl")
...
function objective(θ; λ_val=λ)
    ...
    for cycle in 1:cycles_fit
        for rep in replicates
            data_full = raw_data[rep][cycle]
            data = (index=data_full.index, tH=data_full.tH[1:6], H=data_full.H[1:6], tV=data_full.tV[1:6], V=data_full.V[1:6])
            H0 = data.H[1]
            V0 = data.V[1]
...
for rep in replicates
    for cyc_idx in 1:cycles_sim
        ...
        scatter!(pl_fit[1], data.tH, data.H;
            ...
            markershape=:circle, markersize=8, legend=(0.15, 0.925))
        scatter!(pl_fit[2], data.tV, data.V;
            label=lbl, color=replicate_colors[rep], alpha=0.7,
            ylabel="Virus\n[part/mL]",
            ylims=(1e5, 1e10), yticks=(ytick_vals2, ytick_labels2),
            markershape=:circle, markersize=8, legend=(0.15, 0.37))
    end
end
...
# --- Traits pointillés aux dilutions (jonctions entre cycles) ---
dilution_times = [cycle_tbounds[c][2] for c in 1:cycles_sim-1]

for panel in 1:2
    for td in dilution_times
        vline!(pl_fit[panel], [td];
               color=RGB(31/255, 119/255, 180/255), lw=2, ls=:dot, label = td == dilution_times[1] ? "Dilution" : nothing)
    end
end

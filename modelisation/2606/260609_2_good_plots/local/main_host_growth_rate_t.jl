...
# --- Host growth rate r(1-H/K) from model ---
ytick_vals_growth   = [10.0^i for i in -2:0]
ytick_labels_growth = [L"10^{%$i}" for i in -2:0]
...
for cycle in 1:cycles_sim
    ...
    H_model = sol[1,:]
    growth_rate = r .* (1 .- H_model ./ K)
    lbl = cycle == 1 ? "Model Host growth rate" : ""
    plot!(pl_fit[3], sol.t, growth_rate, label=lbl, color=model_color, lw=4, alpha=0.7,
          ylabel="Host growth rate\n[1/day]", yscale=:log10, ylims=(1e-2, 1e0), yticks=(ytick_vals_growth, ytick_labels_growth),legend=(0.15, 0.17))
end
...
fig_path = joinpath(@__DIR__, "output/host_growth_rate_t.png")

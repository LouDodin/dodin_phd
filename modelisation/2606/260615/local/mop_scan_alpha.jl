...
## ===== Load model =====
const MODEL_FILE = joinpath(@__DIR__, "models/model_SRVi_scan_alpha.jl")
...
const prop_S_0 = 1.0
const t_span   = (0.0, 60.0)
const t_save   = 0.0:0.1:60.0
const α  = 1.6e-5 #1.6179882409883493e-5
e = floor(Int, log10(α))
m = α / 10.0^e
m_str = m ≈ round(m) ? "$(Int(round(m)))" : @sprintf("%.1f", m)
alpha = "$(m_str)e$e"   # → "1.6e-5"

mop_values = 10 .^ range(-1, 10,  length=40)
H0_values  = [1e6] #[1e4, 5e4, 1e5, 5e5, 1e6, 5e6, 1e7, 5e7, 1e8] #[1e6] 

output_path = joinpath(@__DIR__, "output/alpha vs a_b/alpha/test")
mkpath(output_path)
...
## ===== Test find_tR =====
test_cases = [
    (mop=10, H0=1e4),(mop=10, H0=1e5),(mop=10, H0=1e6),(mop=10, H0=1e7),(mop=10, H0=1e8),(mop=10, H0=1e9),
]
...
for (k, tc) in enumerate(test_cases)
    ...
    prob = ODEProblem(ModelDef.ODE_MODEL!, u0, t_span, [α])
...
## ===== Combined figure: 3 subplots (1 row × 3 cols) =====
xlims_      = (1e-1, 1e10)
...
for (sp, (ylab, leg, ydata, title)) in enumerate(zip(ylabels, legends, data_cols, titles))
    ...
    for (k, H0) in enumerate(H0_values)
        ...
        scatter!(pl_combined[sp], res_mop[mask], ydata[mask],
            label            = lbl,
            color            = col,
            markersize       = 7,
...
fig_combined = joinpath(output_path, "$(alpha).png")

...
## ===== Choices =====
const MODEL_FILE  = joinpath(@__DIR__, "models/SV_beta.jl")
...
output_dir = joinpath(@__DIR__, "output/beta/$(n_interior[1])_$(n_interior[2])_$(n_interior[3])_$(n_interior[4])_$(n_interior[5])")
mkpath(output_dir)
...
function decode_theta(θ::Vector{Float64})
    ...
    log_β_func   = build_spline_func(spline_knots, log_β)

    # Fonctions finales : transformation appliquée APRÈS interpolation
    β_func = t -> exp(log_β_func(t))

    # Valeurs aux nœuds pour le plot
    β_vals = exp.(log_β)

    return ϕ, β_func, β_vals
end
...
search_range = vcat(
    [(log_ϕ_lo, log_ϕ_hi)],
    fill((log_β_lo, log_β_hi), N_KNOTS)
)

println("Total parameters: $(N_KNOTS+1)")


## ===== ODE solver =====

function solve_cycle(model!, ϕ, β_func, u0, t0, t1, t_save)
    params = (ϕ, β_func)
...
function objective(θ)
    ...
    for rep in replicates
        for cycle in 1:cycles_fit
            ...
            sol = solve_cycle(ModelDef.ODE_MODEL!, ϕ, β_func, u0, t0_data, t1_data, t_data)
...
ϕ_best, β_func_best, β_knot_vals = decode_theta(θbest)

println("\n===== Best result ($(ModelDef.MODEL_NAME)) =====")
println("  Fitness = ", best_result.fitness)
println("  ϕ       = ", ϕ_best)
println("  β at knots: ", round.(β_knot_vals, sigdigits=3))
...
function raw_rss(θ)
    ...
    for rep in replicates
        for cycle in 1:cycles_fit
            ...
            t_data = sort(unique(vcat(data.tH, data.tV)))
            sol    = solve_cycle(ModelDef.ODE_MODEL!, ϕ, β_func, u0, t_data[1], t_data[end], t_data)
...
ytick_vals_beta   = [10.0^i for i in 0:3]
ytick_labels_beta = [L"10^{%$i}" for i in 0:3]

title_str = "$(ModelDef.MODEL_NAME) - β(t) - ϕ=$(ϕ_best)\n" *
...
for cycle in 1:cycles_sim
    ...
    sol = solve(
        ODEProblem(ModelDef.ODE_MODEL!, u0, (t0, t1), (ϕ_best, β_func_best)),
    ...
    plot!(pl_fit[3], sol.t, β_traj; label=lbl1 ? "β model" : "", color=model_color,  lw=8, alpha=0.8)
end

scatter!(pl_fit[3], spline_knots, β_knot_vals;
    color = :red, markersize = 10, markershape = :circle, label = "β knots",
            common_kw..., ylabel="β\n[part/cell]", yscale=:log10, legend=:topright, ylims=(5e-1, 1e3), yticks = (ytick_vals_beta, ytick_labels_beta)
)
...
open(log_path, "w") do io
    ...
    println(io, "  ϕ         : ", ϕ_best)
    println(io, "  β at knots : ", β_knot_vals)
    println(io)

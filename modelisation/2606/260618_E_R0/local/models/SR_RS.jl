...
const FITTED_PARAMS = [
    (name = :c,   lower = 1e-10, upper = 1, description = "total rate (α+β)"),
    (name = :p,   lower = 1e-10, upper = 1, description = "fraction allocated to α"),
    (name = :ϕ,   lower = 1e-14, upper = 1e-7, description = "interaction rate"),
    (name = :prop_S0, lower = 0.5, upper = 1.1,  description = "proportion of hosts that are susceptible at the beggining of cycle 1"),
]

# ── ODE ─────────────────────────────────────────────────────────────────────
function ODE_MODEL!(dY, Y, p_vec, t)
    c, p, ϕ = p_vec[1], p_vec[2], p_vec[3]
    S, R, Vi = Y[1], Y[2], Y[3]
    H = S + R

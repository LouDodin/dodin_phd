...
const FITTED_PARAMS = [
    ...
    (name = :prop_S0, lower = 1e-5, upper = 1,  description = "proportion of hosts that are susceptible at the beggining of cycle 1"),
]

# ── ODE ─────────────────────────────────────────────────────────────────────
function ODE_MODEL!(dY, Y, p, t)
    c, p, ϕ, λ = p[1], p[2], p[3], p[4]
    S, R, Vi = Y[1], Y[2], Y[3]
    H = S + R
    r, K, β, δ = FIXED_PARAMS.r, FIXED_PARAMS.K, FIXED_PARAMS.β, FIXED_PARAMS.δ
    dY[1] = r*S*(1 - H/K) - ϕ*S*Vi - c*p*S + c*(1-p)*R
    dY[2] = r*R*(1 - H/K) + c*p*S - c*(1-p)*R - λ*ϕ*R*Vi

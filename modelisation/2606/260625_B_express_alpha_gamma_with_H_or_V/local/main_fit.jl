...
ytick_vals4   = [10.0^i for i in -5:-4]
ytick_labels4 = [L"10^{%$i}" for i in -5:-4]
...
for cycle in 1:cycles_sim
    ...
    plot!(pl_fit[2], sol.t, alpha, label= cycle==1 ? "Alpha model" : "", color=model_color, lw=6, alpha=0.7, ylabel="Alpha\n[1/day]", ylims=(1e-5, 1e-4), yticks=(ytick_vals4, ytick_labels4))
    plot!(pl_fit[3], sol.t, V, label= cycle==1 ? "V model" : "", color=model_color, lw=8, alpha=0.7)
    plot!(pl_fit[4], sol.t, gamma, label= cycle==1 ? "Gamma model" : "", color=model_color, lw=8, alpha=0.7, ylabel="Gamma\n[1/day]", yscale=:identity, ylims=(0.009, 0.01))
...
fig_path = "$(output_dir)/$(ModelDef.MODEL_NAME)_fit.png"

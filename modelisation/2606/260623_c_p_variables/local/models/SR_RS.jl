...
const FITTED_PARAMS = [
    (name = :c,   lower = 1e-10, upper = 1-1e-10, description = "total rate (α+β)"),
    (name = :p,   lower = 1e-10, upper = 1-1e-10, description = "fraction allocated to α"),
    (name = :ϕ,   lower = 1e-14, upper = 1e-7, description = "interaction rate"),
]

# ── ODE ─────────────────────────────────────────────────────────────────────
function ODE_MODEL!(dY, Y, p_vec, t)
    c_func, p_func, ϕ = p_vec[1], p_vec[2], p_vec[3]
    c = c_func(t)
    p = p_func(t)
    S, R, Vi = Y[1], Y[2], Y[3]
    H = S + R

...
df = DataFrame(
    ...
    seed         = Int[],
    fitness      = Float64[],
    is_best      = String[],   # "★" si c'est le best_seed de cette combinaison
    export_date  = String[],
)
...
out_csv = joinpath(OUTPUT_ROOT, "summary_fits.csv")
...
println("$(repeat('=', 80))")
@printf("%-4s  %-15s  %-6s  %-6s  %-14s  %-5s\n",
        "rank", "combination", "knots", "seed", "fitness", "flag")
println(repeat('-', 60))
for row in eachrow(df)
    @printf("%-4d  %-15s  %-6d  %-6d  %-14.8e  %s\n",
            row.rank, row.combination, row.total_knots,
            row.seed, row.fitness, row.is_best)
end
println(repeat('=', 80))
println("\nDone : $(now())")
flush(stdout)

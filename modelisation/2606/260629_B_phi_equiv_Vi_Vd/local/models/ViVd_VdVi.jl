...
const FITTED_PARAMS = [
    ...
    (name = :ζ,   lower = 1e-8, upper = 1, description = "Vd->Vi rate"),
]

# ── ODE ─────────────────────────────────────────────────────────────────────
function ODE_MODEL!(dY, Y, p_vec, t)
    ϕ, ε, ζ = p_vec[1], p_vec[2], p_vec[3]
    S, Vi, Vd = Y[1], Y[2], Y[3]
    r, K, β, δ = FIXED_PARAMS.r, FIXED_PARAMS.K, FIXED_PARAMS.β, FIXED_PARAMS.δ
    dY[1] = r*S*(1 - S/K) - ϕ*S*Vi
    dY[2] = β*(1-ε)ϕ*S*Vi - δ*Vi + ζ*Vd
    dY[3] = β*ε*ϕ*S*Vi - δ*Vd - ζ*Vd
end

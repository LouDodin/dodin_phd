...
## ===== Choices =====
const MODEL_FILE = joinpath(@__DIR__, "models/ViVd_VdVi.jl")
...
function objective(θ)
    ...
    for rep in replicates
        ...
        prop_Vi  = 1.0  # fraction of susceptible hosts at cycle start

        for cycle in 1:cycles_fit
            data = raw_data[rep][cycle]
            H0 = data.H[1]
            V0 = data.V[1]
            u0 = ModelDef.INITIAL_CONDITION(H0, V0, prop_Vi)
            ...
            V_pred = [sol(t)[2]+sol(t)[3] for t in data.tV]
            ...
            u_end   = sol.u[end]
            V_end   = u_end[2] + u_end[3]
            prop_Vi  = u_end[2] / V_end
        end
    end
...
for cycle in 1:cycles_sim
    ...
    u0 = ModelDef.INITIAL_CONDITION(H0, V0, prop_Vi_plot)
    ...
    u_end = sol.u[end]
    V_end = u_end[2] + u_end[3]
    global prop_Vi_plot  = u_end[2]  / V_end
    
    S = [u[1] for u in sol.u]
    Vi = [u[2] for u in sol.u]
    Vd = [u[3] for u in sol.u]
    V = Vi+Vd
    ϕ_equiv = [ModelDef.PHI_EQUIV(sol(t), p_best) for t in sol.t]

    plot!(pl_fit[1], sol.t, S, label= cycle==1 ? "H model" : "", color=model_color, lw=8, alpha=0.7)
    plot!(pl_fit[2], sol.t, V, label= cycle==1 ? "V model" : "", color=model_color, lw=8, alpha=0.7)
    plot!(pl_fit[2], sol.t, Vi, label= cycle==1 ? "Vi model" : "", color=:red, lw=6, ls=:dash, alpha=0.7)
    plot!(pl_fit[2], sol.t, Vd, label= cycle==1 ? "Vd model" : "", color=:green, lw=6, ls=:dash, alpha=0.7)
    plot!(pl_fit[3], sol.t, ϕ_equiv, label= cycle==1 ? "ϕ_equiv model" : "", color=model_color, lw=8, alpha=0.7)
end

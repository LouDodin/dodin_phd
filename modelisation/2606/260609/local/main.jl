...
## ===== Select model =====
const MODEL_FILE = joinpath(@__DIR__, "models/model_SRVi.jl")
...
# λ = 0  : free fit on H and V only
# λ > 0  : additionally forces φ_equiv(t) ≈ φ'(t)
const λ = 1
...
function objective(θ; λ_val=λ)
    ...
    total_err = 0.0
    prop_S  = 1.0   # fraction of susceptible hosts at cycle start
    ...
    for cycle in 1:cycles_fit
        for rep in replicates
            ...
            # --- φ ---
            if λ_val > 0
                phi_apparent = [ModelDef.PHI_equiv(sol(t), p) for t in t_phi_grid]
                phi_ref = ϕ_prime.(t_phi_grid)
                err_phi = sum((log.(phi_apparent) .- log.(phi_ref)).^2) / length(t_phi_grid)
...
for cycle in 1:cycles_sim
    ...
    # Panel 4 : S/(S+R) or Vi/(Vi+Vd)
    if ModelDef.MODEL_NAME == "SRVi"
        S_idx = only(filter(sl -> sl.state == :S, ModelDef.STATE_LABELS)).indices[1]
        R_idx = only(filter(sl -> sl.state == :R, ModelDef.STATE_LABELS)).indices[1]
        ...
        Vi_idx = only(filter(sl -> sl.state == :Vi, ModelDef.STATE_LABELS)).indices[1]
        Vd_idx = only(filter(sl -> sl.state == :Vd, ModelDef.STATE_LABELS)).indices[1]

        ratio_ViVd = [
            begin
                Vi = u[Vi_idx]
                Vd = u[Vd_idx]
                (Vi + Vd) > 0 ? Vi / (Vi + Vd) : 0.0
            end
            for u in sol.u
        ]

        plot!(pl_fit[4], sol.t, ratio_ViVd;
            label= cycle == 1 ? "Vi/(Vi+Vd)" : "",
            color=:red,
            lw=3,
            ylabel="Vi/(Vi+Vd)",
            title="Vi/(Vi+Vd)")

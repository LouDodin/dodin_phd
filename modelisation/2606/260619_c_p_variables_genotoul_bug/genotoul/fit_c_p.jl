...
# N_INTERIOR passé en arguments CLI : julia fit_c_p.jl 0 1 2 0 1
# Un nœud SLURM = une combinaison = un appel julia avec 5 entiers en ARGS
# Ces nœuds sont communs à c(t) et p(t).
const N_INTERIOR = length(ARGS) == N_CYCLES ?
    parse.(Int, ARGS) :
    [0, 0, 0, 0, 0]

const OUTPUT_DIR = "/work/user/ldodin/modelo/2606/260619/output/nint_$(join(N_INTERIOR, '_'))"
...
# Bornes des paramètres fittés (espace naturel), reprises du ModelDef SR_RS
const C_LOWER, C_UPPER = 1e-10, 1.0
const P_LOWER, P_UPPER = 1e-10, 1.0
const ϕ_LOWER, ϕ_UPPER = 1e-14, 1e-7
...
function load_replicate(rep::String)::Vector{Cycle}
    ...
    for cyc in 1:N_CYCLES
        ...
        push!(cycles, Cycle(rep, cyc, tH, H, tV, V))
    end
    return cycles
end
...
# Nœuds communs, partagés par c(t) et p(t)
...
# θ = [θ_c (n_knots) ; θ_p (n_knots) ; log(ϕ) (1)]
const IDX_C   = 1:n_knots
const IDX_P   = (n_knots+1):(2*n_knots)
const IDX_PHI = 2*n_knots + 1
const θ_DIM   = 2*n_knots + 1

plog("θ dimension: $θ_DIM  (c: $n_knots, p: $n_knots, ϕ: 1)")


## ===== MODÈLE ODE =====
# États : Y = [S, R, Vi]
# c(t), p(t) passés comme fonctions du temps (issues des splines log-espace).
# ϕ passé comme scalaire constant.

function ode_model!(dY, Y, params, t)
    c_func, p_func, ϕ = params
    S, R, Vi = Y[1], Y[2], Y[3]
    H = S + R
    c = c_func(t)
    p = p_func(t)
    dY[1] = r*S*(1 - H/K) - ϕ*S*Vi - c*p*S + c*(1-p)*R
    dY[2] = r*R*(1 - H/K) + c*p*S - c*(1-p)*R
    dY[3] = β*ϕ*S*Vi - δ*Vi
end

# Condition initiale (cf. ModelDef.INITIAL_CONDITION) :
#   prop_S = fraction de S au début du cycle (=1.0 au cycle 1, puis reportée
#   d'un cycle à l'autre via l'intégration continue).
function initial_condition(H0, V0, prop_S)
    return [prop_S * H0, (1 - prop_S) * H0, V0]
end

function integrate_cycle(cyc::Cycle, params, prop_S::Float64)
    H0 = cyc.H[1]
    V0 = cyc.V[1]
    u0 = initial_condition(H0, V0, prop_S)

    t_data = sort(unique(vcat(cyc.tH, cyc.tV)))
    t0, t1 = t_data[1], t_data[end]

    prob = ODEProblem(ode_model!, u0, (t0, t1), params)
...
function build_params(θ::Vector{Float64}, t_knots::Vector{Float64})
    spline_c = CubicSpline(θ[IDX_C], t_knots)
    spline_p = CubicSpline(θ[IDX_P], t_knots)
    c_func   = t -> exp(spline_c(clamp(t, t_knots[1], t_knots[end])))
    p_func   = t -> exp(spline_p(clamp(t, t_knots[1], t_knots[end])))
    ϕ        = exp(θ[IDX_PHI])
    return (c_func, p_func, ϕ)
end

function objective(θ::Vector{Float64}, t_knots::Vector{Float64})
    params    = build_params(θ, t_knots)
    total_err = 0.0

    for rep in REPLICATES
        prop_S = 1.0  # fraction de S au début du cycle 1

        for cyc in cycles_per_rep[rep]
            sol = integrate_cycle(cyc, params, prop_S)
            sol === nothing && return 1e12

            H_pred = [max(sol(t)[1] + sol(t)[2], 1e-12) for t in cyc.tH]
            V_pred = [max(sol(t)[3],              1e-12) for t in cyc.tV]

            total_err += sum((log.(H_pred) .- log.(cyc.H)).^2) / length(cyc.H)
            total_err += sum((log.(V_pred) .- log.(cyc.V)).^2) / length(cyc.V)

            # Report de la proportion S/(S+R) pour le cycle suivant
            u_end  = sol.u[end]
...
function export_piecewise_spline(io, label, spline, t_knots, units)
    T    = spline.t
    A_sp = spline.u
    z    = spline.z

    @printf(io, "========== PIECEWISE POLYNOMIAL OF log(%s(t)) ==========\n", label)
    @printf(io, "%s(t) = exp(P(t)),  P(t) C² cubic spline over %d intervals\n\n", label, length(T)-1)
...
function export_polynomial(path, t_knots, θbest, best_fit, best_seed, all_results)
    spline_c = CubicSpline(θbest[IDX_C], t_knots)
    spline_p = CubicSpline(θbest[IDX_P], t_knots)
    ϕ_best   = exp(θbest[IDX_PHI])

    open(path, "w") do io
        @printf(io, "========== METADATA ==========\n")
        @printf(io, "model            = SR_RS (c(t), p(t) splines ; ϕ constant)\n")
        ...
        @printf(io, "θ_dim            = %d  (c: %d, p: %d, ϕ: 1)\n", θ_DIM, n_knots, n_knots)
        @printf(io, "n_runs           = %d\n",   N_RUNS)
        @printf(io, "best_fitness     = %.8e\n", best_fit)
        @printf(io, "best_seed        = %d\n\n", best_seed)

        @printf(io, "========== ALL RUNS ==========\n")
        @printf(io, "%-6s  %-14s  %s\n", "seed", "fitness", "θ (log-space) = [θ_c ; θ_p ; log(ϕ)]")
        ...
        @printf(io, "%-4s  %-14s  %-14s  %-14s  %-14s  %-14s\n",
                "i", "t_i (days)", "log(c(t_i))", "c(t_i)", "log(p(t_i))", "p(t_i)")
        θc, θp = θbest[IDX_C], θbest[IDX_P]
        for (i, t) in enumerate(t_knots)
            @printf(io, "%-4d  %-14.4f  %-14.6e  %-14.6e  %-14.6e  %-14.6e\n",
                    i, t, θc[i], exp(θc[i]), θp[i], exp(θp[i]))
        end
        @printf(io, "\n")

        @printf(io, "========== PHI (constant) ==========\n")
        @printf(io, "log(ϕ) = %.6e\n", θbest[IDX_PHI])
        @printf(io, "ϕ      = %.6e  (mL/cell/day)\n\n", ϕ_best)

        export_piecewise_spline(io, "c", spline_c, t_knots, "1/day")
        export_piecewise_spline(io, "p", spline_p, t_knots, "dimensionless, fraction in [0,1]")
    end
end
...
plog("\n========== OBJECTIVE TESTS ==========")
for (c0, p0, phi0) in [(1e-3, 0.5, 1e-12), (1e-2, 0.5, 1e-10), (1e-1, 0.5, 1e-9),
                        (1e-2, 0.1, 1e-8),  (1e-2, 0.9, 1e-7)]
    θ_test = vcat(fill(log(c0), n_knots), fill(log(p0), n_knots), log(phi0))
    try
        f = objective(θ_test, global_knots)
        plog("c=$c0  p=$p0  ϕ=$phi0  →  fitness = $f")
    catch err
        plog("c=$c0  p=$p0  ϕ=$phi0  →  CRASH: $err")
    end
end
...
export_polynomial(poly_path, global_knots, best.θ,

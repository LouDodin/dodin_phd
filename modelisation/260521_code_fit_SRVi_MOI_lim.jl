...
function model!(dY, Y, p, t)
    ...
    if MOI < MOI_lim
        dY[1] = r*S*(1 - (S+R)/K) - ϕ*S*Vi
        dY[2] = r*R*(1 - (S+R)/K)
        dY[3] = β*ϕ*S*Vi - δ*Vi
    else
        dY[1] = r*S*(1 - (S+R)/K) - ϕ*S*Vi - γ*S
        dY[2] = γ*S + r*R*(1 - (S+R)/K)
        dY[3] = β*ϕ*S*Vi - δ*Vi
    end
end
...
display(pl_fit)
savefig(pl_fit, joinpath(@__DIR__, "260521_output/fit_SRVi.png"))